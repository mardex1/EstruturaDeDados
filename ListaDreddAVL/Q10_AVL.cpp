...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string marca, short ano, int &qtd);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string marca, short ano){
    int qtd = 0;
    levantamentoAux(raiz, marca, ano, qtd);
    return qtd;
}

void avl::levantamentoAux(noh* umNoh, string marca, short ano, int &qtd){
    if(umNoh != NULL){
        levantamentoAux(umNoh->esq, marca, ano, qtd);
        if((umNoh->elemento.marca == marca) and (umNoh->elemento.anoFabricacao >= ano)){
            qtd++;
        }
        levantamentoAux(umNoh->dir, marca, ano, qtd);
    }
}
