...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string cultivar, string  marca, int& qtd);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string cultivar, string marca){
    int qtd = 0;
    levantamentoAux(raiz, cultivar, marca, qtd);
    return qtd;
}

void avl::levantamentoAux(noh* umNoh, string cultivar, string marca, int &qtd){
    if(umNoh != NULL){
        levantamentoAux(umNoh->esq, cultivar, marca, qtd);
        if((umNoh->elemento.marcaSemente == marca) and (umNoh->elemento.nomeCultivar == cultivar)){
            qtd = qtd + umNoh->elemento.quantidadeDisponivel;
        }
        levantamentoAux(umNoh->dir, cultivar, marca, qtd);
    }
}
